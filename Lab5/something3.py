...
class Tree:
    ...
    def printw_queue(self, layerc):
        ...
        spaces = [
            2 ** i for i in range(layerc)
        ]  # сколько элементов будет в слое для стандартного бинарного дерева
        ...
        with open("result.txt", "w") as f:
            for i in spaces:
                for j in range(i):
                    ...
                    tab_before = (
                        2 ** (layerc - input_level + len(str(randint_spacecalc)))
                    ) // 2
                    tab_between = (
                        2 ** (layerc - input_level + len(str(randint_spacecalc))) - 1
                    ) // 2
                    ...
                    if len(str(printvar)) >= len(str(randint_spacecalc)):
                        f.write(
                            ...
                            * (
                                tab_between
                                + (len(str(printvar)) - len(str(randint_spacecalc)))
                            )
                            + " " * isminus
                        )
                    else:
