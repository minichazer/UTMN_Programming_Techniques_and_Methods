...
def write_to_file(x):
    with open(f"{x}_output.txt", "w") as file:
        for i in range(100000):
            file.write(
                f"{x} - {''.join(random.choices(string.ascii_uppercase + string.ascii_lowercase, k=m))}\n"
            )
...
print(
    f"Потоки ({n}) завершили запись в файлы ({n}) за {calculate_timings(write_to_file, n)} секунд\n"
)
print(
    f"Потоки ({n}) завершили чтение из файлов ({n}) в один за {calculate_timings(read_from_write_to, n)} секунд\n"
)
